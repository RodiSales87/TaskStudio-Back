...
class LoginController {
  async login(req: Request, res: Response, next: NextFunction) {
    try {
      ...
      const accessToken = TokenRepository.generateAccessToken(user.id, user.role, '5d'); //60s
  ...
  async refresh(req: Request, res: Response, next: NextFunction) {
    try {
      ...
      const acessToken = TokenRepository.generateAccessToken(user.id, user.role, '30s');
